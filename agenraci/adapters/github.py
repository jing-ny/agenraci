"""GitHub adapter — emit applyable governance config, and verify it.

Unlike the humanlayer/langgraph stubs, this target is **real**. `compile` turns a
charter's gates and accountability into three applyable files (a CODEOWNERS, a
repo-ruleset JSON, and setup notes); `verify` reads a repo's live protection and
reports drift from the charter. AgenRACI still does not enforce anything at
runtime — it emits config a human reviews and applies, and checks it stayed in
sync. A charter governs action *types*, not file paths, so the emitted CODEOWNERS
glob is one you scope to your repo.
"""
...
def compile(charter: Charter) -> str:  # noqa: A001 - mirror CLI "compile" verb
    """Emit applyable GitHub governance from the charter (multi-file).

    Three files (split them with ``compile --out-dir``): a real **CODEOWNERS**, a
    GitHub repo-ruleset payload (**github-ruleset.json**), and **github-setup.md**
    with the apply command plus the per-action notes that can't be expressed as a
    ruleset (break-glass, escalation). AgenRACI generates these; it never POSTs
    anything to GitHub — you review and apply them, and `agenraci verify` checks
    they stay in sync.
    """
    ...
    parts = [
        FILE_MARKER.format(path="CODEOWNERS"),
        _codeowners(charter, owners),
        FILE_MARKER.format(path="github-ruleset.json"),
        _ruleset_json(charter, gated),
        FILE_MARKER.format(path="github-setup.md"),
        _setup_md(charter, gated),
    ]
    return "\n".join(parts) + "\n"


def _codeowners(charter: Charter, owners: list[str]) -> str:
    """A real, applyable CODEOWNERS file (not a commented scaffold)."""
    lines = [
        f"# CODEOWNERS for {charter.project} — generated by "
        "`agenraci compile --target github`.",
        "# A charter governs action TYPES, not file paths: scope the glob below to",
        "# the parts of your repo each owner is accountable for. Owners are the",
        "# HUMAN members accountable for a gated action (code owners must be human).",
    ]
    if owners:
        lines.append(f"*    {_handles(owners)}")
    else:
        lines.append("# (no human is accountable for a gated action — set owners by hand)")
    return "\n".join(lines) + "\n"


def _ruleset_json(charter: Charter, gated: dict) -> str:
    """A GitHub repo-ruleset payload (the charter as a FLOOR).

    Require ≥1 approving review and code-owner review on the default branch.
    Per-action nuance (break-glass, escalation, auto-merge) can't all be a ruleset
    rule — github-setup.md carries it. This is a copy-paste artifact; AgenRACI
    never POSTs it.
    """
    rules = []
    if gated:
    ...
    payload = {
        "name": f"agenraci: {charter.project} charter",
        "target": "branch",
        "enforcement": "active",
...
def _setup_md(charter: Charter, gated: dict) -> str:
    """Apply instructions + the per-action notes a ruleset can't express."""
    out = [
        f"# Applying {charter.project}'s charter to GitHub",
        "",
        "`agenraci compile --target github` produced two artifacts next to this "
        "file. AgenRACI never applies them for you — review and apply:",
        "",
        "- **`CODEOWNERS`** — scope the path glob(s), then commit it.",
        "- **`github-ruleset.json`** — apply it (replace `OWNER/REPO`):",
        "",
        "      gh api --method POST repos/OWNER/REPO/rulesets --input github-ruleset.json",
        "",
        "Then keep it honest:",
        "",
        "      agenraci verify --target github CHARTER.yaml --repo OWNER/REPO",
        "",
    ]
    if not gated:
        out.append("No action declares a `gate:`, so nothing requires approval. Add a "
                   "gate to the actions that must never happen unilaterally.")
        return "\n".join(out) + "\n"

    out.append("## Branch protection — per-action notes")
    out.append("")
    out.append("Some nuance (break-glass, escalation) can't be a ruleset rule — "
               "apply it by hand.")
    for name, action in gated.items():
        gate = action.gate
        approvers = _humans_in_role(charter, gate.approver)
        who = (_handles(approvers) if approvers
               else f"role '{gate.approver}' (no human members — assign one)")
        out.append("")
        out.append(f"### {name}")
        out.append(f"- Require a pull request, approved by {who}.")
